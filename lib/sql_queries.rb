...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  #"SELECT projects.title, pledges.amount FROM projects INNER JOIN pledges ON projects.id = pledges.project_id WHERE pledges.amount > projects.funding_goal"
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS difference_over_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING difference_over_goal >= 0"
